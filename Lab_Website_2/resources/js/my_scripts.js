...
//must iterate over var players array
...
//anchor tags using variables

//	purpose: This method will iterate through the stats table and 
					// do the following:
					//	1. Read through each row of the table & determine which team won
						///   the game. // x > y => store x ? y 
...
function loadPlayersPage() {
	...
	for(var i = 0; i < players.length; i++) {
		dropOps += "<a class = 'dropdown-item' href='#' onclick='switchPlayers(" +i+")'>" + players[i].name + "</a>";
	}
	document.getElementById("player_selector").innerHTML = dropOps;
}
...
function switchPlayers(playerNum) {
	...
	document.getElementById("avg_p_yards").innerHTML = Math.round(parseInt(players[playerNum].pass_yards) / parseInt(players[playerNum].games_played));
	document.getElementById("avg_r_yards").innerHTML = Math.round(parseInt(players[playerNum].rushing_yards) / parseInt(players[playerNum].games_played));
	document.getElementById("avg_rec_yards").innerHTML = Math.round(parseInt(players[playerNum].receiving_yards) / parseInt(players[playerNum].games_played));
}
